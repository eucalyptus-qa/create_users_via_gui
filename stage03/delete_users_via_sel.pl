...
print "\n";
print "Running delete_users_via_sel.pl\n";
print "\n";
...
if( is_euca_version_from_memo() == 1 && ($ENV{'QA_MEMO_EUCA_VERSION'} eq "3.1" || $ENV{'QA_MEMO_EUCA_VERSION'} eq "3.0") ){		### FOR 3.1 or before
...
if( $lines =~ /$this_user/ ){
	report_fatal_error("deleting a new user $this_user!!");
}else{
	print "[TEST_REPORT]\tDeleted User $this_user\n";
};
print "\n";
...
###
print "\n";
print "[TEST_REPORT]\tRun of Script delete_users_via_sel.pl is Successful\n";
print "\n";
